<div>
    <form class="auth-register-form mt-2" method="POST" wire:submit.prevent="register">
        <div class="mb-1">
            ...
            <input type="text" class="form-control" wire:model="name" placeholder="{{__('Name')}}" aria-describedby="register-username" tabindex="1" autofocus />
            @error('name') <small class="text-danger">{{$message}}</small> @enderror
        </div>
        <div class="mb-1">
            <label for="register-email" class="form-label">{{__('Email')}}</label>
            <input type="text" class="form-control" wire:model="email" placeholder="{{__('Email')}}" aria-describedby="register-email" tabindex="2" />
            @error('email') <small class="text-danger">{{$message}}</small> @enderror
        </div>

        <div class="mb-1">
            <label for="register-email" class="form-label">{{__('phone')}}</label>
            <input type="number" class="form-control" wire:model="phone" placeholder="{{__('phone')}}" aria-describedby="register-email" tabindex="2" />
